...
parse_multipart(rfile, content_type, content_length):
    - Reads all content_length bytes from rfile into memory; callers may opt into the historical MAX_UPLOAD_BYTES cap (default 50 MiB, env-overridable via HERMES_WEBUI_MAX_UPLOAD_MB), while the chat/workspace and selected integration multipart callers intentionally pass no local size cap
