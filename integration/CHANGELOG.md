...
- **Cron session transcript and turn keys** — 非终态 Cron 会话物化时以 Agent `state.db` 的 active transcript 替换旧的无边界 sidecar 快照，避免上下文压缩后重复追加 user/assistant/tool 消息；首次物化即为真实 Cron prompt 持久化 `turn:1`，并跳过 `context_anchor` 等语义控制行，避免新会话退化显示 `turn:0` 或 turn 编号错位。若 Agent 压缩恢复的唯一真实 user 被插到保留的 assistant/tool 尾部之后，Cron normalization 会将其移回 compaction anchor 后，避免隐藏 anchor 后会话从 assistant 开始。已结束且边界可验证的 execution prefix 还会折叠跨 `compaction_summary` 的同文初始 prompt 恢复副本，使一次 trigger 只保留一个真实初始 turn；边界后的同文 WebUI follow-up 仍是独立 turn，Agent `state.db` 原文不会被改写。fallback 消息与已有 WebUI follow-up 后缀保持原有合并语义。
...
- **Message semantics audit logging** — `hermes_message_semantics` 与 async delegation 状态日志降为 DEBUG，只记录分类、角色、session、delegation 和 turn key 是否存在，不再记录 `content`、`api_content`、完整事件或真实 turn key；移除 `HERMES_MESSAGE_SEMANTICS_LOG_MAX_CHARS`。不改持久化或模型上下文正文。

- **Hermes Agent message semantics** — Agent-generated `internal_scaffold` rows no longer leak into WebUI display transcripts, manifest turn anchors, or artifact settlement. Durable `context_anchor` rows retain their original model-facing content plus explicit semantic fields while remaining hidden from transcript/turn rendering；verification/incomplete 候选在同一回答区域内由最终回答替换，无最终回答时保留最后非空候选，length partial 与续写按顺序合并；legacy verification/pre-verify flags remain recognized.
...
- **Gateway 启动失败恢复** — 全 Profile 首轮启动后，失败、运行时不可用、状态未知或超时的 Profile 会按 5/15/45 秒的有界退避独立重试；已成功的 Profile 不会被重启。WebUI 关闭时先取消等待中的恢复任务，再停止其拥有的 Gateway 子进程，避免退出过程中重新拉起子进程。
