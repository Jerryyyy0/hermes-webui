...
WebUI startup asynchronously ensures every visible Hermes Profile Gateway is running by default. On native hosts, WebUI uses the Agent's authoritative Profile lock and service snapshot: a Gateway confirmed as launchd/systemd-managed is restarted through `hermes [-p <profile>] gateway restart`, so its service manager remains the lifecycle owner; before requesting the restart, WebUI follows newly appended `logs/gateway.log` and `logs/gateway.error.log` records into its own console. A running Gateway whose owner cannot be confirmed is left untouched. When no Gateway is running, WebUI launches and owns a foreground child using `hermes gateway run -v --external-supervisor`. Plain containers replace a pre-existing Profile Gateway with `--replace`, so the new WebUI process becomes the lifecycle owner and forwards its combined stdout/stderr to the same WebUI console sink and persistence destination with a `[gateway:<profile>]` prefix. Direct interactive `python server.py` and container stdout therefore include INFO-and-above Gateway records.
...
- WebUI forwards all stdout/stderr from child Gateways. For native externally owned Gateways, it follows only newly appended `gateway.log` and `gateway.error.log` lines, without replaying historical files, then asks a confirmed Agent service manager to restart the Gateway. Plain containers replace an existing Gateway through the Agent's `--replace` protocol. s6-managed containers are never replaced by WebUI. Gateway lines are redacted before being sent to the WebUI console sink.
