...
- **Chinese approval and clarify display copy** — WebUI approval cards prefer a fork-owned Chinese display description while keeping the Agent's canonical risk keys unchanged for Smart Approval, hooks, and allowlists. When the Agent includes optional `tirith_findings` (`rule_id` / `severity` / `title` / `description`), WebUI localizes known Tirith rules (including `pipe-to-interpreter` and `schemeless-url-in-sink-context`) by `rule_id`; unknown rules keep a Chinese severity prefix plus the original English evidence. Legacy payloads without `tirith_findings` fall back to prose-based display localization. WebUI sessions also instruct `clarify` calls to write questions and choices in Simplified Chinese while retaining literal commands, paths, URLs, and configuration values. Upgrade order: Agent and WebUI may land independently; only newly triggered approvals after both sides are updated get structured findings.
