...
New Cron Hub jobs persist a V1 `workspace_policy`: omitted policy becomes a managed directory beneath the approved WebUI default workspace; `worktree` requires a registered Git base. Jobs written before the field remain legacy, including after ordinary edits; a complete policy submitted by update is the explicit migration action. Each V1 execution carries its new `cron_*` session ID and canonical `cwd` from Agent to WebUI. Only that current-run hand-off may create a `ready` interactive sidecar. History/poll/backfill may replay an existing sidecar, but cannot infer a V1 root from a current job definition. Legacy sessions receive an `external/legacy_shared` continuation binding: trusted historical `cwd` or `workdir` wins, otherwise the selected Profile's approved default workspace is used. This enables follow-up without claiming that fallback directory was the original execution cwd.

`workspace_unverified` applies only when a V1 current-run hand-off is missing or invalid; those records remain viewable but the shared workspace resolver rejects chat follow-up, terminal and file operations. `legacy_shared` records are continuable through their approved external root. Job deletion removes only `ready` sidecars whose root has the managed `sessions/cron/<profile>/<session>` shape; managed roots are deleted directly and worktrees use `git worktree remove`. Failed cleanup leaves the sidecar in place and returns the per-session result, rather than guessing or deleting an unproven directory.
...
- `api/workspace.py` — shared Cron `workspace_unverified` gate in `resolve_session_workspace()`; `legacy_shared` external bindings are permitted
