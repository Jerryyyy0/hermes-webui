...
- **Chinese approval and clarify display copy** — WebUI approval cards prefer a fork-owned Chinese display description while keeping the Agent's canonical risk keys unchanged for Smart Approval, hooks, and allowlists. Known command-risk reasons and common Tirith pipe-to-interpreter findings are localized; unknown Tirith evidence remains visible in its original language. WebUI sessions also instruct `clarify` calls to write questions and choices in Simplified Chinese while retaining literal commands, paths, URLs, and configuration values.
