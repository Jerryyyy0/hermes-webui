...
WebUI startup asynchronously restarts every visible Hermes Profile Gateway by default. On native hosts, WebUI uses the Agent's authoritative Profile lock and service snapshot: a Gateway confirmed as launchd/systemd-managed is restarted through `hermes [-p <profile>] gateway restart`, so its service manager remains the lifecycle owner; before requesting the restart, WebUI follows newly appended `logs/gateway.log` and `logs/gateway.error.log` records into its own console. A running Gateway whose owner cannot be confirmed, or whose state cannot be probed, is replaced through the Agent's `--replace` protocol and then owned by WebUI. When no Gateway is running, WebUI launches and owns a foreground child using `hermes gateway run -v --external-supervisor`. Plain containers use the same replacement path, so the new WebUI process becomes the lifecycle owner and forwards its combined stdout/stderr to the same WebUI console sink and persistence destination with a `[gateway:<profile>]` prefix. Direct interactive `python server.py` and container stdout therefore include INFO-and-above Gateway records.
...
- WebUI forwards all stdout/stderr from child Gateways. For native Gateways confirmed as externally service-managed, it follows only newly appended `gateway.log` and `gateway.error.log` lines, without replaying historical files, then asks that service manager to restart the Gateway. Any unconfirmed native Gateway and every plain-container Gateway is replaced through the Agent's `--replace` protocol. s6-managed containers are restarted through s6 and never replaced by WebUI. Gateway lines are redacted before being sent to the WebUI console sink.
...
- s6 containers retain the service-manager lifecycle and receive `gateway restart`. They are externally owned, are not registered as WebUI child processes, and their logs are outside this forwarding path.
...
- Native hosts, plain containers, and s6 containers enable the WebUI coordinator by default. Plain containers use the same WebUI-owned process path as native hosts; s6 containers retain their `gateway restart` service-manager lifecycle. `HERMES_WEBUI_START_PROFILE_GATEWAYS=0` / `1` remains an explicit override when needed. Agent runtime verification allows 60 seconds per subprocess, and each Profile Gateway cold start has a 300-second readiness window. Starting gateways can activate scheduled model calls and configured messaging/API platforms.
...
- Each all-profile startup pass verifies that runtime once, with the bounded transient retry, then shares the verified invocation with every Profile Gateway. This avoids multiplying cold-start probes by the number of profiles and keeps all profile lifecycle actions on the same resolved runtime.
- The coordinator starts Profile Gateways serially, with the default Profile first. Every child receives a cloned runtime environment pinned to that Profile's `HERMES_HOME`; for named Profiles this is in addition to the explicit `-p <profile>` selector. A child is reported as started only after its own PID holds the Agent runtime lock, its `gateway_state.json` reports `running`, and its event-loop heartbeat is fresh. An early exit or readiness timeout is reported as a failed Profile instead of a false successful start.
...
Cron Hub job deletion remains synchronous: the response is sent after task history is removed. Cleanup resolves the relevant Profiles once, then deletes all matching Cron sessions from each Profile's `state.db` under one cleanup lock, connection, and transaction per Profile. WebUI sidecars, attachments, and the owner Profile's `cron/output/<job_id>/` directory are removed in the same request.

New Cron Hub jobs persist a V1 `workspace_policy`: omitted policy becomes a managed directory beneath the approved WebUI default workspace; `worktree` requires a registered Git base. Jobs written before the field remain legacy, including after ordinary edits; a complete policy submitted by update is the explicit migration action. Each V1 execution carries its new `cron_*` session ID and canonical `cwd` from Agent to WebUI. An in-process hand-off or the exact selected `source=cron` Agent `state.db` record may create a `ready` interactive sidecar; this lets Gateway polling recover after transient in-memory hand-off fields disappear. History/poll/backfill never derives a V1 root from the current job definition alone. An existing unverified sidecar is repaired only before it has a WebUI follow-up; an already continued conversation remains on its approved fallback workspace. Legacy sessions receive an `external/legacy_shared` continuation binding: trusted historical `cwd` or `workdir` wins, otherwise the selected Profile's approved default workspace is used. This enables follow-up without claiming that fallback directory was the original execution cwd.

`workspace_unverified` applies only when a V1 hand-off and its exact Agent `state.db` record cannot provide a valid root. The shared resolver ignores its sidecar path and uses the approved WebUI default workspace for chat follow-up, terminal and file operations; it never treats that default as the original execution root. `legacy_shared` records are continuable through their approved external root. Job deletion removes only `ready` sidecars whose root has the managed `sessions/cron/<profile>/<session>` shape; managed roots are deleted directly and worktrees use `git worktree remove`. Failed cleanup leaves the sidecar in place and returns the per-session result, rather than guessing or deleting an unproven directory.
...
Cron session materialization also persists stable Session Manifest artifact decisions. While a run has no terminal execution boundary, the Agent `state.db` active rows are authoritative for its transcript; rematerialization replaces the prior Agent-owned sidecar snapshot so context compression cannot append duplicate snapshots. Real cron requests receive contiguous stable keys (`turn:1`, `turn:2`, ...) on first materialization, while semantic control rows such as context anchors never consume a key. When Agent compaction restores the execution's only real user after a preserved assistant/tool tail, Cron normalization moves that user directly behind the leading compaction anchor; hiding the anchor therefore still leaves a user-first display transcript. For a verified, completed execution prefix, if a later compaction restores an exact normalized copy of an earlier Cron prompt across a `compaction_summary` anchor, projection retains the first prompt and does not allocate a second turn; Agent `state.db` remains untouched. The rule never crosses `cron_execution_ended_at`, so an equal-text WebUI follow-up suffix remains a separate turn. Missing or invalid execution boundaries leave the transcript unchanged. Hermes Agent's exact max-iteration summary request may be stored as a `role=user` message, but it continues the current cron invocation rather than opening a new Manifest turn. Before a same-ID WebUI follow-up starts, a cron-only prepare gate reconciles the immutable execution prefix, removes that internal boundary, saves those keys without changing session recency, and writes only missing prefix decisions. The gate runs before pending state, journal, SSE channel, or worker creation; a failed save/key check rejects the reply rather than creating a misaligned turn. A successful follow-up then uses the normal streaming artifact path and its next stable key; the final transcript is saved before that new turn's artifact or empty decision. Fallback messages, existing WebUI follow-up suffixes, ordinary WebUI sessions, and historical non-empty cron decisions keep their prior merge behavior.
...
- `api/routes.py` — integration GET/POST dispatch, profiles enrich, static mapping, `__INTEGRATION_SKILLS__`, `__SKILLHUB_ENABLED__`; `GET /api/sessions` filters cron execution rows and injects session status fields when `HERMES_INTEGRATION=1`; `GET /api/session` delegates Cron transcript reconciliation under the session lock; chat start clears old error state and advances the user's own-message read cursor; per-session SSE bridges `SessionChannel` lifecycle frames
