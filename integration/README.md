...
Plain containers without s6/systemd must not use `gateway start`: Hermes treats that command as a successful no-op because the container runtime is expected to own the long-lived process. The project-owned [`run_container_services.sh`](gateway_startup/run_container_services.sh) handles **Profile Gateways only**: it discovers Profiles through `hermes_cli.profiles.list_profiles()`, runs each required `gateway run` process, mirrors its stdout/stderr to the container console while appending per-Profile `logs/gateway.log`, honors multiplex mode, forwards termination, and exits if a managed Gateway exits. It does not locate, configure, or start WebUI.
