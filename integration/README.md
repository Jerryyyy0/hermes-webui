...
- Native hosts, plain containers, and s6 containers enable the WebUI coordinator by default. Plain containers use the same WebUI-owned process path as native hosts; s6 containers retain their `gateway restart` service-manager lifecycle. `HERMES_WEBUI_START_PROFILE_GATEWAYS=0` / `1` remains an explicit override when needed. Starting gateways can activate scheduled model calls and configured messaging/API platforms.
