...
class Session:
    def __init__(self, session_id: str=None, title: str='Untitled',
                ...
                worktree_branch=None,
                 ...
                 async_delegation_cancellation=None,
                 share_token=None,
        ...
        self.async_delegation_cancellation = (
            dict(async_delegation_cancellation)
            if isinstance(async_delegation_cancellation, dict)
            else None
        )
    ...
    def save(self, touch_updated_at: bool = True, skip_index: bool = False) -> None:
        ...
        extra = {k: v for k, v in self.__dict__.items()
                 ...
                 and k not in _placed
                 and k not in ('cron_execution_profile', 'cron_execution_ended_at')
...
def _append_journaled_partial_output(
    ...
    *,
    ...
    def flush_assistant() -> int | None:
        nonlocal appended_any, visible_output_recovered, assistant_parts, reasoning_parts
    ...
    if recovered_tool_calls:
        ...
        appended_any = True
    return visible_output_recovered
    ...
    *,
    ...
    if core_path.exists():
        ...
        if core_messages:
            ...
            initial_core_message_count = len(session.messages)
            ...
            recovered_visible_rows_added = any(
                isinstance(message, dict)
                and message.get('role') == 'assistant'
                and not message.get('_error')
                and bool(str(message.get('content') or '').strip())
                for message in session.messages[initial_core_message_count:]
            )
            if recovered_visible_rows_added or not has_visible_core_answer:
                session.messages.append(
                    _build_recovery_marker_with_retry_hook(
                        recovered_output=recovered_output,
                        stream_id=_stream_id,
...
def _row_may_need_sidecar_metadata_refresh(
    ...
    *,
    ...
    stale_cron_index_row = bool(
        (source == 'cron' or sid.startswith('cron_'))
        and _sidebar_message_count(session) <= 0
    )
    if stale_cron_index_row:
        return True
...
def ensure_cron_project(profile: str | None = None, create: bool = True) -> str | None:
