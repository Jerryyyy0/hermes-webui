...
def resolve_session_workspace(
    ...
    *,
    requested_is_trusted: bool = False,
) -> Path:
    """Resolve a request workspace while preserving managed-session identity.

    Legacy/external sessions retain the existing requested-or-session fallback.
    A managed session may repeat its persisted root, but no caller may replace
    ...
    candidate = requested if requested not in (None, "") else current
    return _resolve_path(candidate) if requested_is_trusted else resolve_trusted_workspace(candidate)
...
def create_managed_workspace(session_id: str, base: str | Path | None = None) -> Path:
    """Create one session-owned directory below the configured workspace base.

    The session ID is server-generated by the caller and is treated as a single
    path component here.  The helper still validates it so future callers cannot
    turn this creation boundary into a traversal primitive.
    """
    if not isinstance(session_id, str) or not re.fullmatch(r"[A-Za-z0-9_-]{1,128}", session_id):
        raise ValueError("Invalid session_id for managed workspace")

    base_path = _resolve_path(base or _BOOT_DEFAULT_WORKSPACE)
    access_error = _workspace_access_error(base_path)
    if access_error:
        raise ValueError(access_error)
    ...
    if os.path.lexists(str(candidate)):
        raise FileExistsError(f"Managed workspace already exists: {candidate}")
    make_anchored_dir(base_path, candidate)
