...
def restart_active_profile_gateway(
    *,
    ...
    try:
        ...
        cmd = list(runtime.command_prefix)
        ...
        if cli_profile is None:
            logger.info(
                "Restarting gateway service via verified Hermes CLI runtime (%s): gateway restart (HERMES_HOME=%s)",
                runtime.kind,
                active_home,
            )
        else:
            logger.info(
                "Restarting gateway service via verified Hermes CLI runtime (%s): --profile %s gateway restart (HERMES_HOME=%s)",
                runtime.kind,
                cli_profile,
                active_home,
            )
