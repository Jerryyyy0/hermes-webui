...
| `HERMES_WEBUI_DEFAULT_WORKSPACE` | `~/workspace` | Default workspace and parent directory for `POST /api/session/new` requests that omit `workspace`; those requests create `<base>/<session_id>`. Docker terminal backends use this managed root only when the same absolute base path is mounted inside the container; other remote terminal backends retain their target-side working directory. Send an explicit `workspace` to retain a shared/project directory. |
...
| `BROWSER_PREVIEW_URL` | *(unset)* | noVNC / remote viewer URL for the workspace iframe (`http://` or `https://`, path and query allowed). Overrides `CAMOFOX_URL` for preview when both are set |
...
| `HERMES_WEBUI_CSP_FRAME_EXTRA` | *(unset)* | Optional space-separated `http(s)://` origins to append to CSP `frame-src` for embedded iframe previews (for example remote VNC viewers) |
