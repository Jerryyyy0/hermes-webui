...
- **`GET /api/integration/webui_login`** — Zhiling Control Plane identity lookup proxy when `HERMES_INTEGRATION=1` and `ZHILING_CONTROL_PLANE_URL` are set. Bearer requests forward to Control Plane and refresh an in-process identity cache; requests without Bearer return the cached identity (no `access_token`). See [`docs/integration-login-api.md`](integration-login-api.md).
- **`POST /api/integration/webui_logout`** — Zhiling auth-proxy user-container logout proxy when `HERMES_INTEGRATION=1` and `ZHILING_LOGOUT_API_URL` (auth-proxy origin) are set; POSTs to `{base}/api/logout`; clears WebUI session cookie, Zhiling identity cache, and returns auth-proxy JSON (e.g. `casdoor_logout_url`). Does not change built-in Sign Out UI.
- **`GET /api/integration/workspace/files`**, **`/file`**, **`POST /api/integration/workspace/file/delete`** — Session-less workspace file list, raw file stream, and file delete under `HERMES_WEBUI_DEFAULT_WORKSPACE` when `HERMES_INTEGRATION=1`. Deleted session artifacts surface as `status: expired` in manifest (see session manifest API).
- **`POST /api/integration/knowledge_base/*`** — Knowledge base downstream BFF proxy when `HERMES_INTEGRATION=1` and `KNOWLEDGE_BASE_URL` are set. Caller supplies `account` / `uuid` in the request body (no identity lookup). Upload is two-step: `upload_docs` then `update_docs`.
