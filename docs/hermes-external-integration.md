...
- **`GET /api/integration/login`** — Zhiling Control Plane identity lookup proxy when `HERMES_INTEGRATION=1` and `ZHILING_CONTROL_PLANE_URL` are set; requires `Authorization: Bearer <access_token>`.
