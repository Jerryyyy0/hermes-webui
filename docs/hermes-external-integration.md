...
- **`POST /api/integration/knowledge_base/*`** — Knowledge base downstream BFF proxy when `HERMES_INTEGRATION=1` and `KNOWLEDGE_BASE_URL` are set. Caller supplies `account` / `uuid` in the request body (no identity lookup). Upload is two-step: `upload-docs` then `update-docs`.
