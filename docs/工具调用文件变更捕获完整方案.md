...
- 由该轮直接工具调用或该轮派生的后台 review 实际提交的 Skill、Memory 等 Hermes 持久状态变化。
...
“所有内置工具”不等于“整台宿主机的所有文件 I/O”。用户可见变更范围固定为 workspace、已登记 Artifact root 和已实际提交的受管 profile 资源；待批准建议记录、Agent 日志、SQLite WAL、浏览器 profile/cache、临时目录、模型缓存和远程系统副作用均属于 operational state，不进入用户 ChangeSet。这些排除必须以资源类型写入 policy，不得靠临时路径模式静默漏掉。
...
因此 Skill/Memory 不应纳入 workspace checkpoint，也不应对整个共享 `HERMES_HOME` 做每轮前后 diff。应在这些原生写入口成功提交实际资源时追加结构化变更事件，并扩展现有 provenance 上下文。待批准建议的创建、更新或删除不属于此捕获范围；批准操作只有在实际提交 Memory 或 Skill 时才产生事件。
...
| 新工具的输出和后台 writer 未接入统一收口 | 可能在最终快照后仍继续写入 | 在统一分发和 process registry 中登记实际启动的 writer，并在最终快照前收敛或降级 |

### 3.1 工具调用记录与运行时收敛

不需要在 `tools.registry.ToolEntry` 增加文件副作用或结算字段。workspace 和 Artifact 的文件列表由 baseline/final 状态差分产生，Memory/Skill 的实际提交由 broker journal 产生；工具名称、参数和结果均不是文件变化证据。

工具 hook 只记录 provenance；process registry 和 background review future 记录实际启动且尚未收敛的 writer。`execute_code`、子代理和后台任务必须传播同一 `TurnCaptureContext`，但这是上下文与生命周期要求，不是工具注册表元数据。

最小规则：

1. 图片、音频、视频、浏览器下载/截图等用户交付文件必须由统一 registrar 归一到已登记 Artifact root。
2. Memory、Skill 等受管资源必须经原生写入口 journal；待批准建议、缓存和日志仍属于 operational state。
3. 已知仍在运行或无法限定写入范围的 Terminal、`computer_use`、子进程等 writer，在最终快照前不能收敛时，本轮至少为 `partial`。
4. MCP 与第三方 plugin 不属于“内置工具完整覆盖”；若其输出不在已登记 root 或无法确认其 writer 已收敛，本轮必须降级。
...
flowchart TD
  ...
  Begin --> Policy["Freeze capture policy and capture roots"]
  Policy --> Baseline["Create stable workspace/artifact baselines"]
  Baseline --> Agent["Run the existing Agent loop"]
  Agent --> Hooks["Record tool calls and track active writers"]
...
2. 不根据工具名、命令文本或参数推导文件列表；范围由 capture policy 和已登记 root 决定，结算由实际 writer 生命周期决定。
3. 文件列表来自 workspace/Artifact root 状态差分。
4. 每个 tool batch 结束后可发布相对整轮 baseline 的 provisional revision；整轮 final diff 是最终权威。
5. hook 提供轮次级 tool provenance；process registry 和后台任务状态提供 writer 收敛证据，均不根据参数猜测变更。
...
`pre_tool_call` 和 `post_tool_call` 将事件传给 Tracker，记录调用 provenance 并关联实际启动的 writer；不得从 `args.path`、terminal command 或 result 提取权威文件变更。
...
还需记录已知 writer 的生命周期状态及其关联的 `task_id`。Artifact root 登记失败、managed-state broker 被绕过、delegated context 丢失或已知 writer 无法收敛时，Tracker 必须将本轮标记为 `partial` 或 `unsupported`。
...
| approval 实际提交 Memory 或 Skill 后 | `memory_committed` / `skill_committed` | 实际提交的最终资源 |
...
待批准建议属于 operational state，不记录为 ChangeSet 或持久状态变更。用户稍后批准造成的真实 Memory/Skill 落盘属于新的实际资源写入事件；该事件使用自己的 operation identity，不回写或篡改原轮次已经结算的 ChangeSet。
...
跨平台首期将 `{HERMES_HOME}/memories`、`skills` 定义为受管状态目录，默认使用 `persistent_write_boundary=cooperative`：

1. `memory`、`skill_manage`、curator 以及会实际提交 Memory/Skill 的 approval 继续复用 Hermes 主进程内的原生写入入口，只在实际资源提交入口增加 TurnWriteContext、资源锁和 journal；
...
5. Terminal、`computer_use` 及其子进程若无法验证写入边界，调用后的轮次至少为 `partial + unbounded_write_scope`。
...
完整性按已声明 capture policy、实际启动的 writer 和适用 journal 计算，而不要求每轮启动所有 adapter。当且仅当所有适用 capture root/journal 都完整，且没有已知未收敛或越出已验证写入边界的 writer 时，才可返回 `complete=true`。
...
跨平台首期默认使用 `checkpoint-worktree`。当本轮使用 Terminal、`computer_use` 或其它无法验证写入边界的工具时，即使 workspace diff 成功也只能返回 `partial + unbounded_write_scope`。这是不引入重型 Runner 的必要产品语义，不得用命令正则或平台 watcher 猜测为 complete。
...
- `.git/**`；
- 待批准建议记录，以及 Agent 日志、capture 自身的 DB/WAL/锁文件、浏览器 profile/cache 和工具临时目录等 `resource_class=operational` 路径；
- 已声明的敏感路径，例如 `.env*`。
...
- `complete`：policy 纳入范围的 baseline/final diff 与适用 journal 完整，且没有已知未收敛或越出已验证写入边界的 writer。
...
`capture_status` 描述 workspace/Artifact roots 的状态捕获；`persistent_capture_status` 独立描述受管 profile 资源的原生事件捕获。`write_boundary` 描述普通文件边界，`persistent_write_boundary` 描述共享 profile 状态边界，不能混用。总字段 `complete=true` 仅在每个适用的 capture root/持久子系统都为 complete、其余子系统为 `not_applicable`，且没有已知未收敛或越界 writer 时成立。`unsupported` 不等于 `not_applicable`。`observed_unattributed`、未收敛 writer、无法验证写入边界或 background review 超时都会使总状态至少为 partial。
...
`turn_change_sets` 是当前最高 revision 的权威快照，`changes_json` 只保存有界的 workspace 净变化；持久状态明细仍以 `managed_state_change_events` 为权威，并由 ChangeSet 按 turn context 聚合。`managed_state_change_events.session_id/turn_id/stream_id` 必须允许为空，因为独立 approval 提交或启动恢复的实际资源写入可能不属于 conversation turn。
...
| `tools/write_approval.py` | 仅在 approval 实际提交 Memory/Skill 时调用对应资源 journal；待批准建议记录属于 operational state |
...
| `agent/tool_executor.py` | 在现有统一分发处传播 TurnCaptureContext，并向 process registry 登记实际启动的 writer；不解析命令推导文件列表 |
...
```json
{
  "turn_file_changes": {
    ...
    "persistent_resources": ["memory", "skills", "cronjob"],
...
同一 profile 的 Memory 与 Skills 是多会话共享状态。原生 journal 能准确记录受控入口的因果来源，但不能让冲突写入同时提交；冲突资源仍需短时加锁。普通工具若可直接写这些目录，则只能发现未归属变化，不能准确归因。待批准建议记录属于 internal operational state，不纳入此共享资源捕获范围。
...
- 浏览器只接收 profile-relative 逻辑路径和必要的状态字段；`before_hash/after_hash` 默认仅留在 Agent 内部，不通过 WebUI API/SSE 暴露。Memory 内容和 Skill 中的秘密文件不得进入事件流。如需跨边界比对，使用服务端密钥 HMAC 或不可逆的随机版本 ID，不直接暴露低熵内容哈希。
- 原生 Memory/Skill 工具只通过 broker 写入；通用 Terminal/脚本在 cooperative 模式下可能绕过该边界，因此一旦调用就失败闭合为 partial，不声称已完整归因。
...
- 新代码优先收敛为 Agent 的 tracker/journal 两个小模块，其余位置只增加必要的生命周期钩子；
...
- 对 File Tool 和子代理强制现有 safe root；Terminal/`computer_use` 无可验证写边界时失败闭合为 partial。
...
- 接入 Memory、Skill 原生实际变更 journal；待批准建议记录保持为 operational state；
...
- 仅在产品后续必须证明变更一定属于该 Agent 轮次，或必须让无法验证写入边界的工具也达到 complete 时，再实施 [轮次级文件事务与归档方案](./轮次级文件事务与归档方案.md) 中的 Runner/shadow work 隔离；
...
- 使用无法验证写入边界的工具且 backend 无可验证隔离时，即使 workspace diff 成功也只能返回 partial；
...
- 待批准建议记录不产生 ChangeSet；approval 只有在实际提交 Memory/Skill 时生成新的资源写入事件；
...
2. 任意内置工具在 policy 范围内产生的最终净变化都能由统一 diff/journal 发现，无需新增工具参数或命令解析器。
3. 已知后台 writer 均进入 process registry 或 turn-owned future；未收敛或无法验证写入边界时不得返回 complete。
4. 每个 tool batch join 后发布累计 provisional revision，整轮 final revision 正确校准删除、恢复、改名和迟到 writer。
5. `complete=true` 仅在所有 capture roots 和适用 journal 均完整结算、且没有已知未收敛或无法验证写入边界的 writer 时成立；此类 writer 必须失败闭合。
...
15. 所有实际提交的受控 Memory、Skill 写入均带 session/turn/tool/origin 因果上下文并先 durable 后发布；待批准建议记录不进入 ChangeSet。
...
- 必须让 Terminal、`computer_use` 等可在任意宿主路径写入的工具仍返回 `complete=true`。

在此之前，checkpoint + writer lifecycle tracking + native journal 的复杂度、跨平台能力、现有代码复用率和产品收益更匹配当前需求。
...
**问题**：敏感内容不只存在于 `.env*`；任意 JSON/YAML/报告都可能包含凭据。Git object 会保留历史内容，低熵 Memory 内容哈希若暴露给浏览器也可被字典猜测。
...
### 19.12 非 conversation 的实际受管写入 identity

**问题**：approval 等操作可由 WebUI、CLI 或启动恢复单独执行。当它实际提交 Memory 或 Skill 时，可能没有新的 `turn_id/stream_id`；强行把该写入回填原轮会篡改已结算事实。

**推荐解法**：

1. 所有实际受管写事件共用 `operation_id`；conversation 操作可另带 turn context，非 conversation 操作的 turn 字段为 null。
2. approval 仅在实际提交 Memory/Skill 时记录新的 operation ID、`actor_type`、`actor_id` 与最终资源 identity；待批准建议记录不产生 ChangeSet 事件。
3. 原轮 ChangeSet 保持不变；后续实际写入在资源历史中独立可查，不修改原轮的 settled revision。
4. API 授权使用 profile + resource identity，不依赖原 conversation session。

**验收要点**：在原轮完成数小时后分别从 WebUI 和 CLI 执行 approval：未实际提交时没有 ChangeSet 事件；实际提交时生成独立幂等事件，原轮结果保持不变。
...
1. workspace/Artifact 文件列表只来自状态差分；managed state 只来自 broker WAL，不从工具名称、参数或输出推导文件列表。
2. 新增会产出用户交付文件的工具必须接入 Artifact root registrar；新增会启动后台 writer 的工具必须接入 process registry 或 turn-owned future；新增实际提交受管状态的工具必须复用 broker journal。
3. `execute_code`/`delegate_task`/background task 传播 TurnCaptureContext 并参与同一 refcount，不创建时间窗口归因。
4. 每个 tool batch join 后持久化相对整轮 baseline 的 provisional revision，最终仍由 conversation final diff 结算。

**验收要点**：覆盖同步 workspace 写入、Artifact 生成、managed state、嵌套工具、后台 writer、无法验证写入边界的工具和 operational-state 排除。任意新增适用工具缺少 registrar、writer lifecycle 或 broker 接入时，对应变更不得被标记为 complete。
...
1. 先明确 workspace、Artifact、managed 和 operational 边界，并让 Artifact registrar、writer lifecycle 与 broker journal 成为新增适用工具的接入点。
...
4. 完成 persistent WAL、资源锁、Skill directory manifest、非 conversation 实际写入 identity 与崩溃恢复。
